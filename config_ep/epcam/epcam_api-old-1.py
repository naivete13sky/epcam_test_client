...
def drill_check(job, step, layers, erf, rout_distance, hole_size, extra_holes, hole_seperation, power_ground_short, missing_hole, 
                npth_to_rout, use_pth, use_npth, use_via, compensated_rout, calculate_board_net, ranges):
    data = {
        'func': 'DRILL_CHECK',
        'paras': [{'job': job}, 
                  {'step': step}, 
                  ...
                  {'calculate_board_net': calculate_board_net},
                  {'ranges':ranges}]
    }
    #js = json.dumps(data)
    #print(js)
...
def layer_compare_point(jobname1, stepname1, layername1, jobname2, stepname2, layername2, 
                        tol = 0.9 * 25400, isGlobal = True, consider_sr = True, map_layer_res = 200 * 25400):
    data = {
            ...
            'paras': {
                        ...
                        'map_layer_resolution': map_layer_res
                      }                    
            }   
    js = json.dumps(data)
    #print(js)
...
def drill2file(job, step, layer,path,isMetric,number_format_l=2,number_format_r=6,
                    zeroes=1,unit=0,x_scale=1,y_scale=1,x_anchor=0,y_anchor=0):
    data = {
        'func': 'DRILL2FILE',
        'paras': {
            'job': job,
...
def output_image(job,step,layers,size,xmin,ymin,xmax,ymax,picpath,picname):
    data = {
        'func': 'OUTPUT_PICTURE',
        'paras': {'job': job, 
                  'step': step,
